...
from transformer4bpm.constants import BPMN2_NAMESPACE, DATA_PATH_TO_CSV_FILES

_logger = logging.getLogger(__name__)

def get_csv_paths(ds_root=DATA_PATH_TO_CSV_FILES) -> List[Path]:
...
class BpmnModelParser:
    def __init__(self, parse_outgoing=False, parse_parent=False, parse_bounds=False, parse_model_name=False):
        self.parse_outgoing = parse_outgoing
        self.parse_parent = parse_parent
        self.parse_bounds=parse_bounds
    ...
    def parse_model_elements(self, csv_paths=None) -> pd.DataFrame:
        ...
        dfs = [self._parse_bpmn_model_elements_csv(p) for p in tqdm(csv_paths, desc="Read csv files: ")]
        df = pd.concat(dfs)
        return df
    ...
    def _parse_df_row(self, row_tuple):
        ...
        if self.parse_model_name:
            return (
                pd.DataFrame.from_records(elements)
                .assign(model_id=row_tuple.model_id,model_name=row_tuple.name,organization_id=row_tuple.organization_id)
            )
        else:
            return (
                pd.DataFrame.from_records(elements)
    ...
    def _get_elements_flat(self, model_dict) -> List[Dict[str, str]]:
        ...
        while len(stack) > 0:
            ...
            if self.parse_bounds:
                record["bounds"]=element["bounds"]
            #if self.parse_model_name:
            #    record['model_name']=element['model_name']
