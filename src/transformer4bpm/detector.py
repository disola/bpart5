...
class ModelLanguageDetector:
    ...
    def add_detected_natural_language_from_meta(self, df_meta):
        df_meta['detected_natural_language'] = [self._get_text_language(name) for name in tqdm(df_meta.name,desc="Detect language of models: ")]

    def get_detected_natural_language_from_bpmn_model(self, df):
        df_labels = get_df_models_and_labels(df, " ")
        df_labels['detected_natural_language'] = [self._get_text_language(label) for label in tqdm(df_labels.label,desc="Detect language of models: ")]
        return df_labels